...
[![Release](https://img.shields.io/badge/release-v0.4.0--cross--platform-emerald)](../../releases)
[![License](https://img.shields.io/badge/license-Apache--2.0-green)](#license)
[![Backend](https://img.shields.io/badge/print%20backends-IPP%20%7C%20CUPS%20%7C%20Windows-teal)](#打印后端)
[![Self-Test](https://img.shields.io/badge/self--test-18%2F18%20scenarios%20passing-brightgreen)](#开发与测试环境mock--virtual)
...
# 2. Host 守护进程（REST :3001 / Realtime :3002 / Virtual IPP :3061 + TLS :3063 / Virtual eSCL Scanner :3065）
...
│ └──────────────────────────────────────────────────────────┘│
│ ┌─────────▼ Virtual eSCL Scanner :3065（验证真实 eSCL 扫描）─┐ │
│ │ vscan-flatbed / vscan-adf 两档案 + _uscan._tcp 自通告   │ │
│ └──────────────────────────────────────────────────────────┘│
...
├── src/components/ops/            # 10 视图 + store + widgets（含扫描页）
...
- 自动化测试（18 场景：10 Mock + 1 控制台鉴权 + 5 IPP/mDNS（含 ipps TLS）+ 2 eSCL 扫描（全链路 + PDF 导出），含 Host 重启任务恢复）：`POST /api/tests/run` 或 Web 控制台「调试 · 开发测试」页一键运行，报告落盘 `test-runs/`。
