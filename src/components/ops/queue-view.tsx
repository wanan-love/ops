      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <Select value={stateFilter} onValueChange={setStateFilter}>
                <SelectTrigger className="h-8 w-32" aria-label="按状态筛选">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <ScrollArea className="max-h-[32rem] rounded-md border">
            <Table className="[&_th]:h-11 [&_th]:text-xs [&_th]:font-semibold [&_th]:uppercase [&_th]:tracking-wide [&_td]:py-3">
              <TableHeader className="sticky top-0 z-10 bg-card/95 backdrop-blur">
                <TableRow className="hover:bg-transparent">
                  <TableHead className="w-36">任务</TableHead>
                  <TableHead>文档 / 来源</TableHead>
                  <TableHead className="w-36">打印机</TableHead>
                ...
                {filtered.map((job, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="max-w-36 truncate text-xs">{printer?.name ?? job.printerId}</TableCell>
                      <TableCell>
                        ...
                        <div className="flex justify-end gap-1">
                          {canCancel && (
                            <Button
                              size="sm"
                              variant="ghost"
                              className="h-8 text-muted-foreground transition-colors duration-200 hover:text-destructive max-sm:size-11"
                          ...
                          {canRetry && (
                            <Button
                              size="sm"
                              variant="ghost"
                              className="h-8 transition-colors duration-200 max-sm:size-11"
        ...
        {job ? (
          <>
            ...
            <div className="mt-4 space-y-5 px-4 pb-8">
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-2 rounded-lg border bg-muted/20 p-3 text-xs">
                <Detail label="打印机" value={printer?.name ?? job.printerId} />
                ...
                {job.error && (
                  <div className="col-span-2 rounded border border-destructive/40 bg-destructive/10 px-2 py-1.5 text-destructive">{job.error}</div>
                )}
